...
def preprocess_data(
    ...
    # 3.1 - mnar
    log_preprocessing_step("Applying MNAR treatment", logger=logger)

    data = treat_mnar(data, inplace=True)
    ...
    data[numeric_columns] = data[numeric_columns].mask(
        data[numeric_columns].abs() > float32_max, np.nan
    )
